      ...
      for (int cell = 0; cell < InvadersInGroup[row].length; cell++) {
        if (InvadersInGroup[row][cell] != null) {
          if (InvadersInGroup[row][cell].alive == false) {
            ...
            if(pos.y + (cellSize + spacing.y) * row > height && curPlr != null){
             curPlr.alive = false; 
            }
          }
...
class InvaderGrunt {
  ...
  void takeDamage(float damageTaken) {
    float cashToReceive = min(health,damageTaken);
    health -= damageTaken;    
    PLR_CASH += round(cashToReceive * calculateScoreMult());

    TagStorage = (Tag[]) append(TagStorage, new Tag(pos,str(cashToReceive),color(255,200,200),15,20));

    if (health <= 0) {
      alive = false;
      enemiesKilled++;
      ExplosionStorage = (ExplosionFX[]) append(ExplosionStorage, new ExplosionFX(pos, 50, 60));
      TagStorage = (Tag[]) append(TagStorage, new Tag(pos,"KOx" + str(enemiesKilled),color(255,255,200),24,10));
